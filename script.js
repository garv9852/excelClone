...
$(document).ready(function () {
    ...
    $(".input-cell").click(function (e) {
        ...
        let ro = $(this).attr('data').split('-');
        let [rowid, colid] = getRowCol(this);
        $(".formula-editor.selected-cell").text(ro[1] + rowid);
        $('.formula-editor.formula-input').text($(this).text());
        $(this).addClass("selected");
        $('.input-cell.selected').keypress(function (event) {
            ...
            if (keycode == '13') {
                ...
                $(`#row-${rowid+1}-col-${colid}`).addClass('selected').attr("contenteditable", "true").focus().trigger('click');
                //$(`#row-${rowid+1}-col-${colid}`).focus();
            }
        });
        $(".input-cell.selected").on('keydown', function (e) {
            var keyCode = e.keyCode || e.which;
            if (keyCode == '9') {
                e.preventDefault();
                $(this).removeClass('selected').blur();
                $(`#row-${rowid}-col-${colid+1}`).addClass('selected').attr("contenteditable", "true").focus().trigger('click');
            }
        });
        $(".input-cell.selected").on('keydown', function (e) {
            var keyCode = e.keyCode || e.which;
...
$(".icon-cut").click(function () {
    $(".input-cell.selected").each(function () {
        window.copySheet = selectedSheet;
        check = 'cut';
...
$(".icon-paste").click(function () {
    let pasteCell = [];
    $(".input-cell.selected").each(function () {
        pasteCell = getRowCol(this);
        return false;
    });
    let diff = [];
    for (let i in selectedCells) {
        let r = selectedCells[i][0];
        let c = selectedCells[i][1];
        if (i == 0) {
            diff.push(pasteCell[0] - r);
            diff.push(pasteCell[1] - c);
        }
        let xr = r + diff[0];
        let xc = c + diff[1];
        let data;
        data = $(`#row-${r}-col-${c}`).text();
        if (window.copySheet != selectedSheet) {
            data = cellData[window.copySheet][r][c].text;
            if (check == 'cut') {
                delete cellData[window.copySheet][r][c];
            }
        } else if (check == 'cut') {
            delete cellData[selectedSheet][r][c];
            $(`#row-${r}-col-${c}`).text("");
        }
        $(`#row-${xr}-col-${xc}`).text(data).addClass("selected");
        let newcellData = cellData;
        if (!newcellData[selectedSheet][xr]) {
            newcellData[selectedSheet][xr] = {};
        }
        newcellData[selectedSheet][xr][xc] = {
            ...defaultProperties
        };
        newcellData[selectedSheet][xr][xc].text = data;
        cellData = newcellData;
    }
    selectedCells = [];
    console.log(cellData);
